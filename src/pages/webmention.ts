...
const WM_PROPERTIES = ['mention-of', 'in-reply-to', 'like-of', 'repost-of', 'bookmark-of'];

const FAKE_CONTENT: Record<string, string> = {
  'in-reply-to': 'Really enjoyed this post! The approach you described here is exactly what I was looking for.',
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    return new Response('source and target must be valid URLs', { status: 400 });
  }
  ...
  try {
    await env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `)
      .bind(source, targetPath, fake.wm_property, fake.author_name, fake.author_url, fake.author_photo, fake.content_text, fake.published)
      .run();
  } catch (e) {
...
export const GET: APIRoute = () =>
  new Response('This is the webmention endpoint. Send a POST request with source and target fields.', {
    status: 200,
    headers: { Allow: 'POST' },
  });
